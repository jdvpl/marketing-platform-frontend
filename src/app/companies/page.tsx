...
export default function CompaniesPage() {
  ...
  const handleDeleteCompany = async (id: string) => {
    if (confirm(t('companies_confirm_delete'))) {
    ...
    <ProtectedRoute>
      <DashboardLayout>
        ...
        <div className="mb-8 flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">{t('companies_title')}</h1>
            <p className="mt-2 text-gray-600">{t('companies_desc')}</p>
          </div>
          <button
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 bg-white rounded-lg shadow-md">
            ...
            <p className="text-gray-600 mb-4">{t('companies_no_companies')}</p>
            <button
              onClick={() => setShowCreateModal(true)}
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        {t('companies_created')} {new Date(company.createdAt).toLocaleDateString()}
                      </p>
                    </div>
                  </div>
                </div>
        ...
        {showCreateModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-8 max-w-md w-full mx-4">
              <h2 className="text-2xl font-bold text-gray-900 mb-6">{t('companies_modal_title')}</h2>

              <form onSubmit={handleCreateCompany} className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    {t('companies_name_label')}
                  </label>
                  <input
                    type="text"
