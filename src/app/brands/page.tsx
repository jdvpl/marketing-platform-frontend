...
export default function BrandsPage() {
  ...
  // Keep companyId in sync
  useEffect(() => {
    if (selectedCompanyId) {
      setNewBrand((prev) => ({ ...prev, companyId: selectedCompanyId }));
    }
  }, [selectedCompanyId]);
  ...
  const handleCreateBrand = async (e: React.FormEvent) => {
    ...
    const result = await dispatch(createBrand({ ...newBrand, companyId: selectedCompanyId }));
    if (createBrand.fulfilled.match(result)) {
      setShowCreateModal(false);
      setNewBrand({ name: '', category: '', language: 'es', tone: 'casual', companyId: selectedCompanyId });
      dispatch(fetchBrands(selectedCompanyId));
    }
  };
